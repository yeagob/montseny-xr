...
const VoxelGame: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    // Mobile / Email State
    const mobileInput = useRef({ x: 0, y: 0, isLooking: false, lookX: 0, lookY: 0, lastLookX: 0, lastLookY: 0 });
    ...
    useEffect(() => {
        // More reliable mobile detection - avoid false positives on touch-capable PCs
        const isMobileDevice = () => {
            ...
            // Check for small screen (mobile/tablet typical)
            const isSmallScreen = window.innerWidth <= 1024;

            // Combine: must have mobile OS OR (touch support AND small screen)
            return isMobileOS || (('ontouchstart' in window) && isSmallScreen);
        };
                    ...
                    <div
                        ...
                        onTouchMove={(e) => {
                            if (mobileInput.current.isLooking) {
                                ...
                                mobileInput.current.lookX = dx * 0.02;  // 4x faster for mobile responsiveness
                                mobileInput.current.lookY = dy * 0.02;  // Was 0.005, now 0.02
                                mobileInput.current.lastLookX = e.touches[0].clientX;
                                mobileInput.current.lastLookY = e.touches[0].clientY;
                            }
                        }}
